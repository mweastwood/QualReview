...
\newthought{Stars are categorized} by a spectral type and a luminosity class.
Spectral types are (from hottest to coolest) O, B, A, F, G, K, M, L, T,\sidenote{
    Pneumonic: Oh be a fine girl kiss me lovingly tonight
} where L and T
...
\newthought{You will need to} be able to draw and explain an HR diagram.
...
\newthought{The equation of hydrostatic equilibrium} (Equation~\ref{eq:hydrostatic_equilibrium})
...
\newthought{The radiative temperature gradient} (Equation~\ref{eq:radiative_transport})
...
\newthought{In addition to} the four main equations of stellar structure,
...
The equation of state is typically taken to be polytropic\sidenote{
    $P = K\rho^\gamma$ with $\gamma=5/3$ for an ideal, monatomic gas.  Note that $\gamma\leq 4/3$
    is unstable.
}, ideal gas\sidenote{
    $P = \frac{\rho kT}{\mu m_p}$
}, or radiation pressure\sidenote{
    $P = \frac{1}{3}aT^4$
}.

Here is a brief review of the mean molecular weight $\mu$.
...
\subsection{Scaling Relations and Homology on the Main Sequence}\label{sec:scaling_relations}
\newthought{Homology is used to} find scaling relations between properties of a star,
...
\begin{enumerate}
	...
	\item Is the luminosity carried by radiative diffusion
          ($\kappa \propto T^{4} L^{-1} M^{-1} R^{4}$)\sidenote{
                This scaling relation comes from solving for $\kappa$ in
                Equation \ref{eq:radiative_transport}.
          } or convection ($T \propto M R^{-1}$)\sidenote{
                From the adiabatic temperature gradient, Equation \ref{conv}
          }?
...
The following homology relationships were solved for using a quick python code
($\texttt{homology.py}$)\sidenote{
    Feel free to play around with the code, I committed it to the git repo.
    It's pretty poorly commented, but come talk to me if you want an explanation of the output.
...
\newthought{Astrophysics in a Nutshell} gives the following scaling relations on the main sequence:
\begin{dgroup}
...
\newthought{Here's an alternative} treatment of the scaling relationships.
...
\begin{enumerate}
	...
	\item Radiative energy transport\sidenote{This scaling relation comes from solving for $\kappa$ in Equation \ref{eq:radiative_transport}, and approximating $dT(r)/dr$ as $T/R$}: $\kappa \propto T_{c}^{4} L^{-1} M^{-1} R^{4}$
\end{enumerate}
...
\newthought{Cool note} on the Henyey Track -- if the mass of the protostar is too low,
