...
\subsection{Compton Scattering}\label{sec:compton}
...
Hund's rules describe how to determine the lowest-energy spectroscopic
level.  A level is described by $^{2S+1}L_J$, where 2S+1 is the spin
multiplicity, L is total orbit angular momentum, and J is the total
angular momentum.  The rules are that the lowest energy level is the
level with:

\begin{itemize}
  \item The lowest S
  \item For the same S, the lowest L
  \item For the same S and L, the lowest J if the subshell is more
    than half filled, and the highest J otherwise
\end{itemize}

Consider an electron configuration of $1s^2 2s^2 2p^6 3s^2 3p^2$.  It
can be shown that the only possible terms and their corresponding
levels are $^1{D}_2$, $^3{P}_{0,1,2}$, and $^1{S}_0$.  Using the first
of Hund's rules, we determine that the ground level is either
$^1{D}_2$ or $^1{S}_0$.  Using the second, we determine that it is
$^1{S}_0$.  Since there is only one possible J, the third rule is not
required.  If there were more to choose from, we would choose the
highest possible J, since the outer p subshell can contain 6 electrons
but has only 2.
