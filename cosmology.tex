...
\begin{equation}
\rho_{rad} = \rho_{rad,0}\frac{a_0^4}{a^4}
\end{equation}

The corresponding matter density is:
\begin{equation}
\rho_mc^2 = \rho_{m,0}c^2\frac{a_0^3}{a^3}
\end{equation}

The two energy densities are equal when:
\begin{equation}
\frac{a_0}{a} = \frac{\rho_{m,0}c^2}{\rho_{rad,0}}
\end{equation}

We can calculate the present day radiation density using $\rho_{rad} = aT^4$, where $T = T_{CMB}$.  The present day matter density is apparently not far from the critical density.  We get:

\begin{equation}
\frac{a_0}{a} = \frac{0.3\rho_cc^2}{1.7\rho_{rad,0}} = 3500
\end{equation}

The 1.7 factor in front of the radiation density accounts for the energy density due to the cosmic neutrino background.  

400,000 years, $z \sim 1100$, $T \sim 3700K$, 0.3 eV:  Recombination.  At this time, the universe cooled to the point where the formation of neutral hydrogen was energetically favorable.  Shortly after, the photons decoupled from matter in the universe (surface of last scattering).  Prior to recombination, photons were primarily coupled to matter through the reaction:

\begin{equation}
H + \gamma \rightharpoonup \leftharpoondown p + e^-
\end{equation}

Using the Saha equation, by plugging in an ionization fraction of 0.5 for hydrogen, the temperature and redshift at which this occurs in the universe is approximately $T \sim 4000 K$ and $z \sim 1500$.  

However, in the real universe recombination is not an equilibrium process.  Recombination to the ground state of hydrogen doesn't work because the universe is optically thick to photons with energies of 13.6eV and higher (these photons would just go ionize a recombined hydrogen atom).  What happens is recombination to excited states, then decay to the ground state, emitting a non-ionizing photon.  
...
\newthought{First of all}, I highly recommend Wayne Hu's website\sidenote{
    \url{http://background.uchicago.edu/~whu/intermediate/intermediate.html}
}.  He's a CMB theorist at Chicago, and his website is a very good introduction
...
Beyond the total radiation density, the CMB provides a lot of information about the universe
through the fluctuations we see in the CMB.  Some of these fluctuations are primordial\sidenote{
    That is, they reflect fluctuations in the initial conditions of the universe.
}, some of these fluctuations are not cosmological\sidenote{
    For example, there exists a dipole fluctuation due to the motion of the Milky Way with
    respect to the CMB.  Note that this motion is not the only source of dipole fluctuations.
}, but some of these fluctuations are due to sound waves that propagated in the early universe.
...
was quite low\sidenote{
    Recall that the rate of recombinations scales as $n^2$, while the rate of
    ionizations scale as $n$.  Therefore at lower densities, the equilibrium between recombinations
    and ionizations shifts towards a higher ionization state.  This means that a lower temperature
    is necessary for the universe to recombine at low densities.
}.
...
\newthought{The first peak} in the CMB power spectrum (see Figure~\ref{fig:cmb_power_spectrum})
...
Now the dispersion relation for a (linear) sound wave is given by
\begin{dmath}
    \omega^2 = c_s^2k^2
\end{dmath}.
...
\newthought{The second peak} of the CMB power spectrum occurs precisely where we expect it to
...
must correspond to a rarefaction\sidenote{
    You can see this by drawing harmonics.  Put amplitude on the $y$-axis and time on the $x$-axis.
    All the sound waves begin at a minimum (maximal rarefaction).
    A peak in the power spectrum occurs when a sound
    wave of a given frequency is at a maximum or a minimum when $t=t_{\rm recombination}$.
    Notice that the fundamental frequency ends at a point of maximal compression, and the
    first harmonic ends at a point of maximal rarefaction.  Ask Michael to draw this for you
    if you need help visualizing it.
}.  Now although the baryons are coupled to the radiation during these acoustic oscillations,
...
\newthought{The damping tail} of the CMB power spectrum at small angular scales\sidenote{
    $l > 1000$
} is related to the diffusion of photons.  This process is called Silk damping.
...
Hopefully now at the end of this discussion you should be able to explain most of the variation
...
One of the reasons we know black holes exist at the centers of AGN is 
...
Closely tied to galaxy formation is supermassive black hole formation.  It is 
...
reionization (as indicated by WMAP data)\sidenote{
    Actually, WMAP only tells us that the average redshift of reionization was $z\sim 10$.
    Observations of Gunn-Peterson troughs in high-redshift quasars indicate a neutral fraction
    $X_{\rm HI} > 10^{-4}$ at $z\sim 7$, so an extended period of reionization seems likely.
    The redshift of Pop III star formation is only known from simulations.
}.  While population III stars were likely very bright in the UV, they are theoretically
...
host galaxies, leading to a partial recombination\sidenote{
    It is expected that the number of UV photons required to reionize the universe is of
    order 1-2 times the number of hydrogen atoms that needed to be ionized.  The process
    of reionization is very photon starved, and recombinations were, for the most part,
    negligible.
}.

It is unlikely that Pop III stars could have reionized the universe on their own (due to their
...
the gas temperature again\sidenote{
    This produces the absorption feature the LWA will be looking for.
}.  Meanwhile, X-rays heat the gas, raising its temperature above the CMB temperature\sidenote{
    This produces an emission feature that PAPER, the MWA, and LOFAR will essentially
    be looking for.
}.  As reionization procedes, the 21 cm transition eventually disappears due to a lack of neutral
hydrogen.
